...
Overall, in assessing credit risk, we want to ensure that we detect everyone that is high-risk, even if there will be false positives, than incorrectly characterizing a loan application to be low-risk. This means that high sensitivity for the high-risk assessment is more important than precision in the detection process. Amongst the models, the Easy Ensemble AdaBoost Classifier gave the highest sensitivity (recall) at 92%. That model also had a high balanced accuracy score, at 93.2%. However,the precision for a high_risk assessment is low, sitting at 9%. This means that a positive classification is not that reliable and when processing many different applications, this lack of precision can mean a lot of extra work, leading to inefficiency. The rest of the models have low sensitivity, which is not ideal in making sure that we don't accidentally characterize high_risk as low_risk profiles. Therefore, I would suggest that none of these models be used.
